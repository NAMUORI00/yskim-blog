...
> 소유확인 코드는 `src/config.ts`의 `SITE.googleSiteVerification`,
> `SITE.naverSiteVerification`에 입력하면 `<head>`에 자동으로 meta 태그가
...
6. **요청 → RSS 제출**에 `index.xml`(RSS 피드) 등록 — 네이버는 RSS 수집을 활용합니다.
