...
test("archive pages render post lists inside content-window controls", async () => {
  ...
  assert.match(postListWindow, /<article class="readme-card archive-window" aria-label=\{`\$\{label\}: \$\{title\}`\} data-content-window data-archive-window>/);
...
test("site css adds home hero and archive structure with tablet overrides", async () => {
  ...
  assert.match(css, /\.archive-window\s*\{[^}]*display:\s*flex/s);
  assert.match(css, /\.archive-window\s*\{[^}]*flex-direction:\s*column/s);
  ...
  assert.match(ruleContaining(css, ".post-feed.archive-window__feed"), /padding:\s*var\(--space-4\)/);
  assert.match(css, /\.archive-window__pagination\s*\{[^}]*border-top:\s*1px solid var\(--line\)/s);
  assert.match(css, /\.archive-window__pagination\s*\{[^}]*padding:\s*var\(--space-3\)\s*var\(--space-4\)/s);
  assert.match(css, /\.readme-card\.is-maximized > \.archive-window__pagination\s*\{[^}]*flex:\s*0 0 auto/s);
